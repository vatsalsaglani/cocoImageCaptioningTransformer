...
from AttentionModules import DecoderLayer, PositionalEncoding
...
class Decoder(nn.Module):
    ...
    def __init__(
        self, target_vocab_size, emb_dim, layers, heads, dim_key, dim_value,
        dim_model, dim_inner, pad_id, dropout = 0.1, num_pos = 200
        ):

        super().__init__()

        self.word_embedding = nn.Embedding(target_vocab_size, emb_dim, padding_idx = pad_id)
        self.position_encoding = PositionalEncoding(emb_dim, num_pos = num_pos)

        self.dropout = nn.Dropout(dropout)

        self.layer_stack = nn.ModuleList([
            DecoderLayer(dim_model, dim_inner, heads, dim_key, dim_value, dropout = dropout)
    ...
    def forward(self, target_seq, target_mask, encoder_output, source_mask, return_attentions = False):
